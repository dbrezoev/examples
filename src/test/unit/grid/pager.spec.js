...
describe('testing pager(no dependent to data store):', () =>{
	...
	it('should get visible pages and check next/prev buttons avaiability', done =>{
		...
		expect(pager.pageCount).toBe(8);
		expect(pager.pages).toEqual([4, 5, 6, 7, 8]);
		expect(pager.prevDisabled).toBeFalsy();
		expect(pager.nextDisabled).toBeTruthy();

		pager.update(1, 20, 75);
		expect(pager.pageCount).toBe(4);
	...
	it('should change next/prev pages until cap is reached', done => {
		...
		expect(pager.page).not.toBe(curPage);
		pager.prev();
		expect(pager.page).toBe(curPage + 1);
		pager.next();
		expect(pager.page).toBe(curPage + 2);
		pager.next();
		expect(pager.page).toBe(curPage + 3);
		pager.next();
		expect(pager.page).toBe(curPage + 4);
		pager.prev();
		expect(pager.page).toBe(curPage + 3);
	...
	it('should change nextJump/prevJump pages until cap is reached', done => {
		...
		pager.prevJump();
		expect(pager.page).toBe(curPage - 5);
		pager.nextJump();
		pager.nextJump();
		pager.nextJump();
		pager.nextJump();
		pager.nextJump();
		expect(pager.page).toBe(curPage + 15);
		done();
	});
